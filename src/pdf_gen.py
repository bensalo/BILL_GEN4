...
class mypdf:
    ...
    def merge_modules_data_and_save(self,data):
        ...
        mypdf.create_kundendaten(self, self.pdf, Name, Adresse, Ort)
        mypdf.create_betreff(self, self.pdf, Betreff, rn)	
        mypdf.create_briefinhalt(self, self.pdf, Briefinhalt, Betrag, Name)
        self.pdf.save()
    ...
    def create_briefinhalt(self, pdf, briefinhalt, Betrag, name):
        pdf.setFont("Helvetica", 12)
        text = pdf.beginText(30, 450)
        text.textLine("Liebe " +  name + ",")
        text.textLine("")
        text.textLine(briefinhalt + " bitte ich Dich, " + Betrag + " Euro auf mein Konto zu überweisen.") 
        text.textLine("")
        text.textLine("")   
        text.textLine("")    
